...
import 'package:saverp_app/database/transaksi_DAO.dart';
  ...
  TransactionBloc() : super(TransactionInitial()) {
    ...
    on<AddTransaction>((event, emit) async {
      ...
      if (state is TransactionLoaded) {
        ...
        final updatedTransaction =
            List<Transaction>.from(currentState.transaction)..add(updatedTag);
    ...
    on<UpdateTransaction>((event, emit) async {
      await TransactionDAO.updateTransaction(
          event.transaction, event.expenseCategory, event.incomeCategory);
      if (state is TransactionLoaded) {
        final currentState = state as TransactionLoaded;
    ...
    on<DeleteTransaction>((event, emit) async {
      ...
      if (state is TransactionLoaded) {
        final currentState = state as TransactionLoaded;
        final updatedTransaction = currentState.transaction
